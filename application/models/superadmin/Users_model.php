...
class Users_model extends CI_Model
{
	...
	{
		$hsl = $this->db->query("INSERT INTO user(name_user,email_user,password_user,level_user,status_user,photo_user) 
		VALUES ('$nama','$email',MD5('$pass'),'$level','1','$gambar')");
		return $hsl;
	}

	function insert_user_noimg($nama, $email, $pass, $level)
	{
		$hsl = $this->db->query("INSERT INTO user(name_user,email_user,password_user,level_user,status_user) 
		VALUES ('$nama','$email',MD5('$pass'),'$level','1')");
		return $hsl;
	}

	function update_user_nopass($id_user, $nama, $email, $level, $gambar)
	{
		$hsl = $this->db->query("UPDATE user SET name_user='$nama',email_user='$email',level_user='$level',photo_user='$gambar' 
	...
	{
		$hsl = $this->db->query("UPDATE user SET name_user='$nama',email_user='$email',password_user=MD5('$pass'),level_user='$level',
		photo_user='$gambar' WHERE id_user='$id_user'");
		return $hsl;
	}
